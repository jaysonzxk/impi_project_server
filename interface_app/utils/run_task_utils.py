...
class RunTaskUtils:
    ...
    def __save_interface_result(self, interface, success, response, version):
        ...
        if interface.parameter_type == 'text':
    ...
    def __assert_text(self, response, assertion):
        ...
        key = assertion['key']
        if key in response:
            return True if assertion['include'] == 'yes' else False
        else:
            return False if assertion['include'] == 'yes' else True

    def __assert_json(self, response, assertion):
        """
        json类型断言, 使用jsonPath语法
        :param response:
        :param assertion:
        :return:
        """
    ...
    def run(self):
        ...
        interface = self.__get_task_interface()
        for i in interface:
